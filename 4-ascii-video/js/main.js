...
function initCaptureDevice() {
  try {
    vid = createVideo('../../audio/16788_1699161603.mp4',vidLoad);
    // vid.size(640, 480);
...
function vidLoad() {
  vid.loop();
  vid.volume(1);
  vid.style('pointer-events', 'auto');
}

function setup() {
  // noCanvas();
  createCanvas(540, 780); // we need some space...
...
function draw() {
  if(vid !== null && vid !== undefined) { // safety first
    background(0);
    /*
    ...
    */
    gfx.background(0);
    gfx.image(vid, 0, 0, gfx.width, gfx.height);
    /*
    ...
    */
    if(showOryginalImageFlag) image(vid, 0, 0, width, height);
    /*
...
function mousePressed() {
  // Play the video when the user clicks the mouse
  if (vid ) {
    vid.play();
  }
}
